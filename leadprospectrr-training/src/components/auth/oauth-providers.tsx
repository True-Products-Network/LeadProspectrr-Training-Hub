...
export function OAuthProviders({ onProviderClick, loading }: OAuthProvidersProps) {
    ...
    async function fetchProviders() {
      try {
        ...
        if (response.ok) {
          ...
          // Fallback to Google only if API fails
    ...
    <div className={`grid gap-3 ${providers.length === 1 ? 'grid-cols-1' : 'grid-cols-2'}`}>
      {providers.map((provider) => (
        <Button
          key={provider.id}
          variant="outline"
          onClick={() => onProviderClick(provider.id as 'google' | 'github')}
          disabled={loading}
          className="h-11"
        >
